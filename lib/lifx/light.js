...
Light.prototype.sendWithPromise = function(packetObj,ack) {
  var self = this;
  return new Promise(function(resolve,reject) {
    var sendopts = {};
    sendopts.callback = function(err,seq,lite) {
      if(!err) {
        resolve({seq:seq,light:self});
      } else {
        reject(err);
      }
    }
    if(typeof ack==='object') {
      if(ack.retries) sendopts.retries = ack.retries;
      if(ack.timeout) sendopts.timeout = ack.timeout;
    }
    self.client.send(packetObj,sendopts);        
  });
}
 ...
 */
Light.prototype.off = function(duration,ack) {
  ...
  if(ack !== undefined)
    return this.sendWithPromise(packetObj,ack);
  else
 ...
 * @param {Number} [duration] transition time in milliseconds
 */
Light.prototype.on = function(duration,ack) {
  if (duration !== undefined && typeof duration !== 'number') {
    throw new RangeError('LIFX light off method expects duration to be a number');
  }
 ...
 * @param {Number} [duration] transition time in milliseconds
 * @param {Object} [ack] if option present, the method returns a Promise which fulfills when the light sends an acknowledgement
 * @param {Number} [ack.timeout] defaults to DEFAULT_TIMEOUT if not set
 * @param {Number} [ack.retries] defaults to DEFAULT_RETRIES if not set               
 */
Light.prototype.color = function(hue, saturation, brightness, kelvin, duration,ack) {
