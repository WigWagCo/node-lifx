...
var ackEntryHash = function(addr,seq) {
  return ""+addr+":"+seq;
}
...
function AckEntry(opts) {
  this.retryRemain = 0;
  this.timeout = 0;
  this.inqueue = 1; // when created, one in the queue
  ...
  this.socket.bind(opts.port, opts.address, function() {
    ...
    this.packetPump = function() {
      if (this.messagesQueue.length > 0) {
        ...
        if(msg.onSent) msg.onSent();
        this.socket.send(msg.data, 0, msg.data.length, this.port, msg.address);
        this._dbg('D - ' + msg.data.toString('hex') + ' > ' + msg.address + (msg.ackRequired ? (" want ACK ("+msg.sequence+")")  : ""));
      } else {
        if(this.sendTimer) {
...
Client.prototype.send = function(msg,opts) {
  ...
  if(opts && typeof opts === 'object' && typeof opts.callback === 'function') {
    ...
    var hash = ackEntryHash(packet.address,this.sequenceNumber);
    entry.packet = packet;
    this.ackTable[hash] = entry;

    var timeoutHandler = function(hash){
      if(this.ackTable[hash] === undefined) {
        this._dbg("Lost entry in ackTable. ignoring. ("+hash+")");
        return;
      }
      this._dbg("TIMEOUT on ack, seq: " + hash);
      if(this.ackTable[hash].retryRemain < 1) {
        if(typeof this.ackTable[hash].callback === 'function') {
          var err = new Error('timeout');
          err.hash = hash;
          this.ackTable[hash].callback.call(undefined,err,hash,this);
        }
        this._dbg("Removing ACK entry: " + hash);  
        delete this.ackTable[hash];
      } else {
        this._dbg("RETRY on seq: " + hash + " remain: " + this.ackTable[hash].retryRemain);
        if(this.ackTable[hash].inqueue < 1) {
          this.ackTable[hash].retryRemain--;
          this.ackTable[hash].inqueue++;
          this.messagesQueue.unshift(this.ackTable[hash].packet);
          this.ackTable[hash].timer = setTimeout(timeoutHandler,this.ackTable[hash].timeout);
          this.startSend();
        } else {
          this._dbg("pack: " + hash + " already in queue.");
        }
      }
    }.bind(this,hash); 

    entry.onAck = function(hash){
      clearTimeout(this.ackTable[hash].timer);
      this._dbg("Got ACK on seq: " + hash);
      if(typeof this.ackTable[hash].callback === 'function') {
        this.ackTable[hash].callback.call(undefined,undefined,hash,this);
      }
      this._dbg("Removing ACK entry: " + hash);
      delete this.ackTable[hash];
    }.bind(this,hash);

    packet.onSent = function(hash,handler) {
      if(this.ackTable[hash]) {
        this.ackTable[hash].inqueue--;
        this._dbg("Timer on seq " + hash + " " + myopts.timeout);
        this.ackTable[hash].timer = setTimeout(handler,myopts.timeout);  // start the timer when the packet is really sent...
      } else {
        this._dbg("Lost entry in ackTable. onSent() ignoring. ("+hash+")")
      }
    }.bind(this,hash,timeoutHandler);
...
Client.prototype.processAckTable = function(msg, rinfo) {
  var hash = ackEntryHash(rinfo.address,msg.sequence);
  var entry = this.ackTable[hash];
  if(entry) {
    this._dbg("FOUND ACK entry for " + hash);
    entry.onAck();
    return true;
  } else
    return false;
...
Client.prototype.light = function(identifier) {
  ...
  // Id does not contain dots or colons b  ip
