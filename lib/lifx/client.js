  ...
  if(opts && typeof opts === 'object' && typeof opts.callback === 'function') {
    ...
    var timeoutHandler = function(seq){
      if(this.ackTable[seq] === undefined) {
        this._dbg("Lost entry in ackTable. ignoring.");
        return;
      }
      this._dbg("TIMEOUT on ack, seq: " + seq);
      if(this.ackTable[seq].retryRemain < 1) {
        ...
        this._dbg("RETRY on seq: " + seq + " remain: " + this.ackTable[seq].retryRemain);
    ...
    packet.onSent = function(seq,handler) {
      if(this.ackTable[seq]) {
        this._dbg("Timer on seq " + seq + " " + myopts.timeout);
...
Client.prototype.processAckTable = function(msg, rinfo) {
  ...
  if(entry) {
    this._dbg("FOUND ACK entry for " + msg.sequence);
    entry.onAck();
    return true;
  } else
    return false;
